...
bool BootShutdownService::initialize() {
  ...
  if (ret < 0) {
    syslog(LOG_ERR,
           "Failed to prepare to accept connections on socket FD. %s\n",
           strerror(errno));
    shutdown();
    return false;
  }
...
void BootShutdownService::prepareShutdown() {
  ...
  std::thread thread([this] {
    {
      boost::process::child c("/bin/sh", "-c",
                              "systemctl stop jetson-container.service");
      c.wait();
    }
    {
      boost::process::child c("/bin/sh", "-c",
...
void BootShutdownService::isReadyToShutdown() {
  ...
  int ret =
      write(connection_, out_stream.str().c_str(), out_stream.str().length());
  if (ret < 0) {
