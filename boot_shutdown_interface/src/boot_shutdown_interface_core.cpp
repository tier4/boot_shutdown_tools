...
BootShutdownInterface::BootShutdownInterface()
  ...
  srv_prepare_shutdown_ = create_service<PrepareShutdown>(
      ...
      std::bind(&BootShutdownInterface::onPrepareShutdown, this, _1, _2),
      rmw_qos_profile_default);

  srv_execute_shutdown_ = create_service<ExecuteShutdown>(
      "~/service/execute_shutdown",
      std::bind(&BootShutdownInterface::onExecuteShutdown, this, _1, _2),
      rmw_qos_profile_default);
...
void BootShutdownInterface::onPrepareShutdown(
  ...
  preparation_child_ = boost::process::child(
      "/bin/sh", "-c",
      fmt::format("sync; echo 3 > /proc/sys/vm/drop_caches; sleep {}",
                  preparation_delay_));
  ...
  response->power_off_time = builtin_interfaces::msg::Time(
      get_clock()->now() + rclcpp::Duration(preparation_delay_, 0));
}
...
void BootShutdownInterface::onExecuteShutdown(
  ...
  shutdown_child_ =
      boost::process::child("/bin/sh", "-c", fmt::format("shutdown -h now"));
...
bool BootShutdownInterface::isPreparationTimeout() {
  return (get_clock()->now() - preparation_start_time_) >
         rclcpp::Duration(preparation_timeout_, 0);
}
